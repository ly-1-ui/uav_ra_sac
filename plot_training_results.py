...
#！！！更改有/无旋转：修改def plot_convergence(result_path='./SAC_model/')处的路径
...
def plot_convergence(result_path='./SAC_model/'):
    """
...
def evaluate_and_plot_best_model(result_path='./SAC_model/'):
    ...
    # === 修改后的正确写法 ===
    # 使用 bs_pos 变量画基站
    ax.scatter(bs_pos[0], bs_pos[1], bs_pos[2], color='g', marker='^', s=100, label='Base Station (BS)')
    
    # 起点保持不变，读取 logs 里的第一个点
    ax.scatter(logs['uav_x'][0], logs['uav_y'][0], logs['uav_z'][0], color='k', marker='o', label='Start')
    
    # 使用 uav_target_position 变量画紫色的终点星星
    ax.scatter(uav_target_position[0], uav_target_position[1], uav_target_position[2], color='m', marker='*', s=150, label='End Point')
